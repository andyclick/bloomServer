...
return buf.str();
 ...
 string format_timestamp(struct tm tm) {
     char timespan[100];
     timespan[0]=0;
    sprintf(timespan,"%d-%02d-%02d %02d:%02d:%02d", tm.tm_year+1900, tm.tm_mon+1,tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
...
//
    ...
    urlnode->taskid = 9;
    urlnode->id = 11370360615634767874LLU;
...
//
 ...
 Thrift::Client *client = new Thrift::Client("localhost", 38080);
  run(client);
  string html;
  char *filename="111.txt";
  html=readFileIntoString(filename);
  FILE * f = fopen("111.txt","rb+");
  fseek(f,0,SEEK_END);
  int filelen = ftell(f);
  rewind(f);
  ...
  cells.push_back(make_cell("k100", "col", 0, strlisturl, "2012-11-15 17:22:23"));
...
bool check_fetch_url(char * url)
{
    bool ret=false;
     Thrift::Client *client = new Thrift::Client("192.168.19.11", 38080);
     HqlResult result;
     Namespace ns = client->namespace_open("corextendedfetch");
     char sql[100];
     sql[0]=0;
     sprintf(sql,"select * from memorytable where row='%s'",url);
     client->hql_query(result, ns,sql);
     client->namespace_close(ns);
...
{
    int content_len=100;
    if(check_fetch_url(url))
    {
      struct  timespec time1;
      clock_gettime(CLOCK_REALTIME, &time1);
      long realtime=time1.tv_nsec;
      struct tm *local;
      time_t t;
      t=time(NULL);
      local=localtime(&t);
      string now=format_timestamp(*local);
      char timespan[100];
      timespan[0]=0;
      //rintf(timespan,"%lld",t);
      sprintf(timespan,"%lld",realtime);
      //Thrift::Client *client = new Thrift::Client("localhost", 38080);
      Thrift::Client *client = new Thrift::Client("192.168.19.11", 38080);
      std::vector<Hypertable::ThriftGen::Cell> cells;
      if (!client->namespace_exists("corextendedfetch"))
            client->namespace_create("corextendedfetch");
       Namespace ns = client->namespace_open("corextendedfetch");
      //Mutator m = client->mutator_open(ns, "insert_thrift_test1", 0, 0);
      Mutator m = client->mutator_open(ns, "content_tbl", 0, 0);
      string key=string(timespan)+string(":")+string(url);
      string key1=string(url)+string(":")+string(timespan);
      cells.push_back(make_cell(key1.c_str(), "content", 0, content,now.c_str() ));
      cells.push_back(make_cell(key1.c_str(), "content_type", 0,"utf-8",now.c_str() ));
      cells.push_back(make_cell(key1.c_str(), "status", 0,"1",now.c_str() ));
      char llen[100];llen[0]=0;
      sprintf(llen,"%d",content_len);
      cells.push_back(make_cell(key1.c_str(), "len", 0,llen,now.c_str() ));
      client->mutator_set_cells(m, cells);
      client->mutator_close(m);

      std::vector<Hypertable::ThriftGen::Cell> cells0;
      Mutator m0 = client->mutator_open(ns, "fetch_tbl", 0, 0);
      cells0.push_back(make_cell(key.c_str(), "spider_id","spider_id","1",now.c_str() ));
      cells0.push_back(make_cell(key.c_str(), "spider_id","index","0",now.c_str() ));
      cells0.push_back(make_cell(key.c_str(), "spider_source_id","source_id","1",now.c_str() ));
      cells0.push_back(make_cell(key.c_str(), "spider_source_id","spider_id","1",now.c_str() ));
      cells0.push_back(make_cell(key.c_str(), "status",0,"1",now.c_str() ));
      cells0.push_back(make_cell(key.c_str(), "isnew",0,"0",now.c_str() ));
      client->mutator_set_cells(m0, cells0);
      client->mutator_close(m0);


      std::vector<Hypertable::ThriftGen::Cell> cells1;
      Mutator m1 = client->mutator_open(ns, "memorytable", 0, 0);
      key=string(url);
      cells1.push_back(make_cell(url, "source_id","source_id","1",now.c_str() ));
      cells1.push_back(make_cell(url, "source_id","index","1",now.c_str() ));
      cells1.push_back(make_cell(url, "source_batch_id","source_id","1",now.c_str() ));
      cells1.push_back(make_cell(url, "source_batch_id","batch_id","1",now.c_str() ));
      cells1.push_back(make_cell(url, "source_batch_id","index","1",now.c_str() ));
      cells1.push_back(make_cell(url, "duplicate",0,"1",now.c_str() ));
      client->mutator_set_cells(m1, cells1);
      
      client->mutator_close(m1);
      client->namespace_close(ns);
    }
}
...
void get_content_hypertable()
{
    ...
    client->hql_query(result, ns, "select content from content_tbl");
    //out <<result << std::endl;

    client->hql_query_as_arrays(result_as_arrays, ns, "select * from content_tbl");
     //out << result_as_arrays << std::endl;
    client->namespace_close(ns);

    char * key=const_cast<char *>((result.cells[0]).key.row.c_str());
    char * value=const_cast<char *>((result.cells[0]).value.c_str());
    get_decompress(value);
}
