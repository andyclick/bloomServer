...
static CURLcode process_zlib_error(char *data, z_stream *z)
{
	if (z->msg) {
		errorlog("Error while processing content unencoding.\n%s", z->msg);
	} else {
		errorlog("Error while processing content unencoding.Unknown failure within decompression software.");
	}
	return CURLE_BAD_CONTENT_ENCODING;
}

static CURLcode
exit_zlib(z_stream *z, int *zlib_init, CURLcode result)
{
  inflateEnd(z);
  *zlib_init = 0;
  return result;
...
CURLcode
deflate_write(char *data, z_stream *k, size_t nread, Buffer *buffer)
{
  int status;                   /* zlib status */
  CURLcode result = CURLE_OK;                   /* Curl_client_write status */
  char decomp[DSIZ];            /* Put the decompressed data here. */
  z_stream *z = k;         /* zlib state structure */
  int zlib_init = 1;
  ...
  for (;;) {
    ...
    if (status == Z_OK || status == Z_STREAM_END) {
      if (DSIZ - z->avail_out) {
        add_buffer(buffer, decomp, DSIZ - z->avail_out);
        
        return exit_zlib(z, &zlib_init, result);
      }
      ...
      if (status == Z_STREAM_END) {
        if (inflateEnd(z) == Z_OK)
          ...
          return exit_zlib(z, &zlib_init, process_zlib_error(data, z));
      }
...
GZIPNUM check_gzip_header(unsigned char const *data, size_t len, size_t *headerlen)
{
  int method, flags;
...
CURLcode
gzip_write(char *data, z_stream *z, size_t nread, int & zlib_init,  Buffer *buffer)
{
	int status;                   /* zlib status */
	CURLcode result = CURLE_OK;		    /* Curl_client_write status */
	char decomp[DSIZ];            /* Put the decompressed data here. */
  ...
  if (zlib_init == 1) {
    ...
    switch (check_gzip_header((unsigned char *)data, nread, &hlen)) {
    case GZIP_OK:
      z->next_in = (Bytef *)data + hlen;
      z->avail_in = nread - hlen;
      zlib_init = 3; /* Inflating stream state */
      break;
    ...
    case GZIP_UNDERFLOW:
      ...
      if (z->next_in == NULL) {
        return exit_zlib(z, &zlib_init, CURLE_OUT_OF_MEMORY);
      }
      memcpy(z->next_in, data, z->avail_in);
      zlib_init = 2;   /* Need more gzip header data state */
    ...
    memcpy(z->next_in + z->avail_in - nread, data, nread);
    ...
    case GZIP_OK:
      ...
      z->next_in = (Bytef *)data + hlen + nread - z->avail_in;
