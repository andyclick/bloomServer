...
static HashEntry hash_lookup(HashTable table, const void *key, int key_len, int value, 
			     int *foundp, int add);
...
{
    return hash_lookup(table, key, key_len, 0, 0, 0);
}

HashEntry hash_find_or_add(HashTable table, const void *key, int key_len, int value,
			   int *foundp)
{
    return hash_lookup(table, key, key_len, value, foundp, 1);
}
			     ...
			     int *foundp, int add)
{
	...
	*foundp = (*entry != 0);
    ...
    {
	rehash(table);
	return hash_lookup(table, key, key_len, value, foundp, add);
    }
...
void hash_remove(HashTable table, const void *key, int key_len)
{
	...
	if(key_compare((*e)->key, (*e)->key_len, key, key_len) == 0)
	{
        HashEntry entry = *e;
	    *e = (*e)->next;
	    Free(entry->key);
...
void *Malloc(int bytes)
{
    ...
    //if(!mem)
        //mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "malloc failed - %s:%s:%d:%d\n" ,ERROR_LOG_SUFFIX);
    return mem;
}
...
void *Realloc(void *mem, int bytes)
{
    ...
    //if(!mem)
       // mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "realloc failed - %s:%s:%d:%d\n" ,ERROR_LOG_SUFFIX);
    return mem;
}
