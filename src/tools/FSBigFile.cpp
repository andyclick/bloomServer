...
{
  //int fd = creat(name, S_IREAD | S_IWRITE);
  int fd = open(name, O_CREAT | O_WRONLY, S_IREAD | S_IWRITE);
  if (fd < 0) {
...
FSBigFile::FSBigFile(const char *chunkListFile, const std::vector<std::string>* dirlist, char mode)
{
  ...
  if (mode != 'r' && dirlist == NULL) {
      mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "Invalid parameter for FSBigFile constructor - %s:%s:%d", INFO_LOG_SUFFIX);
  }
  ...
  if ((rc = mg->open(*this, chunkListFile, mode)) < 0) {
    if (mode == 'r' || rc != RC_BF_CANNOT_OPEN_FILE) {
      mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "Cannot open FSBigFile %d - %s:%s:%d",rc ,INFO_LOG_SUFFIX);
    }
    if ((rc = mg->create(chunkListFile)) < 0) {
      mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "Cannot create FSBigFile - %s:%s:%d",INFO_LOG_SUFFIX);
    }
    if ((rc = mg->open(*this, chunkListFile, mode)) < 0) {
      mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "Cannot open FSBigFile - %s:%s:%d",INFO_LOG_SUFFIX);
    }
  }
}
...
int FSBigFile::add_chunk()
{
  ...
  chunkName += name;
  chunkName += '.';
  char sizestr[16] ;sizestr[0] = 0;
  int2str(file.size(), sizestr, 10, 0);
  chunkName += sizestr;
  ...
  std::ofstream os(main_file.c_str());
...
int FSBigFile::seek(int64_t pos, int whence)
{
 ...
 seek_forward:
  for (i = seek_from; i < (int)file_size.size(); i++) {
    if (pos < file_size[i]) {
...
chunk_manager::chunk_manager(const std::vector<std::string>* dirlist)
{
  ...
  for (int i = 0; i < (int)dirlist->size(); i++) {
    ...
    if (stat(dir_list[i].c_str(), &buf) < 0) {
      mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "Error in reading disk list for FSBigFile - %s:%s:%d",INFO_LOG_SUFFIX);
    }
    if (S_IFDIR == S_IFMT & buf.st_mode) {
      mylog_fatal(m_pLogGlobalCtrl->veryimportantlog, "Invalid disk specification for FSBigFile - %s:%s:%d",INFO_LOG_SUFFIX);
    }
  }
}
...
int chunk_manager::open(FSBigFile& file, const char* filename, char mode)
{
  ...
  if (!is.is_open()) {
    ...
    mylog_error(m_pLogGlobalCtrl->errorlog, "can't open FSBigFILE %s - %s:%s:%d",filename,INFO_LOG_SUFFIX);
    goto error_exit;
  }
  try {
      is >> chunkName;
    ...
    mylog_error(m_pLogGlobalCtrl->errorlog, "can't read from is chunkName = %s file size = %d - %s:%s:%d",chunkName.c_str(), file.file.size(),INFO_LOG_SUFFIX);
  }
  if (file.file.size() <= 0) {
	int rc;
    file_num = 0;
  ...
  for (i = 0; i < (int)file.file.size(); i++) {
    struct stat buf;
    if (stat(file.file[i].c_str(), &buf) < 0) {
      rc = RC_BF_CANNOT_OPEN_FILE;
      mylog_error(m_pLogGlobalCtrl->errorlog, "can not open file in state - %s:%s:%d",file.file[i].c_str(),INFO_LOG_SUFFIX);
      goto error_exit;
    }
  ...
  if (strcmp(tmp_mode, "w") == 0) {
      mylog_info(m_pLogGlobalCtrl->infolog, "write mode open  %s - %s:%s:%d",filename,INFO_LOG_SUFFIX);
  }
  cur_file = fopen(s, tmp_mode);
  if (cur_file == NULL) {
    rc = RC_BF_CANNOT_OPEN_FILE;
    mylog_error(m_pLogGlobalCtrl->errorlog, "can not open file in state %s- %s:%s:%d", s,INFO_LOG_SUFFIX);
    goto error_exit;
  }
  ...
  //errorlog("Opened file number %d %s for FSBigFile\n", file_num, file.file[file_num].c_str()); 
