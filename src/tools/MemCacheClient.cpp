 ...
 */    
#define MEMCACHECLIENT_RECONNECT_SEC    1
#define MAXBUF  1024
#define ADDRLEN  sizeof("aaa.bbb.ccc.ddd:PPPPP")
...
static bool select_fd (int fd, int maxtime, int wait_for)
{
    ...
    tmout.tv_sec = (long) maxtime/1000;
    tmout.tv_usec = (maxtime % 1000) * 1000;

    do {
        result = select (fd + 1, rd, wr, NULL, &tmout);
    } while (result < 0 && errno == EINTR);

    if (result <= 0) //timeout or occure error
    {
        mylog_error(m_pLogGlobalCtrl->errorlog, "MemcacheClient in select_fd %d %d %d %d %d - %s:%s:%d",fd, maxtime, wait_for,result, errno,INFO_LOG_SUFFIX);        
        return false;
    }
...
ServerSocket::Disconnect()
{
    ...
    while (rc != SOCKET_ERROR && rc > 0) {
        ...
        {
          errno = 0;
          ret = select_fd(mSocket, mTimeout, WAIT_FOR_READ);
        ...
        while (rc <= 0 && (errno == EINTR || errno == EAGAIN) && ret && trynum++ < 20);
    }
    ...
    )
{
    ...
    try {
        ...
        if (rc != 0) {
            //int errno1 = GetLastSocketError();
            //errorlog("errno %d in connect\n", errno1);
            if (rc != SOCKET_ERROR || (GetLastSocketError() != EWOULDBLOCK && GetLastSocketError() != EINPROGRESS && GetLastSocketError() != EINTR)) throw rc;
            ...
            rc = select(s+1, &fds, &wr, NULL, &timeout);
            //rc = select(s+1, &fds, 0, 0, &timeout);
            //errorlog("errno %d in connect over ret= %d \n", errno, rc);
            if (rc == 0 || rc == SOCKET_ERROR) throw rc;
        }
        ...
        rc = setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (const char*) &timeout, sizeof(timeout));
        if (rc != 0) throw rc;

        rc = setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (const char*) &timeout, sizeof(timeout));
        if (rc != 0) throw rc;
        */
        mSocket = s;
        mylog_info(m_pLogGlobalCtrl->infolog, "INFO: now socket %d - %s:%s:%d",mSocket ,INFO_LOG_SUFFIX);
    ...
    )
{
    ...
    {
      errno = 0;
      ret = select_fd(mSocket, mTimeout, WAIT_FOR_WRITE);

      if (ret) {
          count = send(mSocket, a_pszBuf, (int) a_nBufSiz, 0);
      }
    }
    while (count <= 0 && (errno == EINTR || errno == EAGAIN) && ret && trynum++ < 20);

    if (count <= 0) {
        mylog_error(m_pLogGlobalCtrl->errorlog, "MemcacheClient in send %d trynum %d errno %d - %s:%s:%d", count, trynum ,errno,INFO_LOG_SUFFIX);
        Disconnect();
...
loop:
    ...
    if (trynum++ < 20 && n == -1 && (errno == EINTR || errno == EAGAIN)) {
        errorlog("errno %d in send %d\n", errno, n);
        errno = 0;
        goto loop;
    }

    errorlog("errno %d in send out %d\n", errno, n);
    ...
    ) 
{
    ...
    do
    {
      ...
      if (ret) {
          count = recv(mSocket, a_pszBuf, a_nBufSiz, 0);
      }
    }
    while (count <= 0 && (errno == EINTR || errno == EAGAIN) && ret && trynum++ < 20);

    if (count <= 0) {
        mylog_error(m_pLogGlobalCtrl->errorlog, "MemcacheClient in recv %d trynum %d errno %d - %s:%s:%d", count, trynum,errno ,INFO_LOG_SUFFIX);        
        Disconnect();
...
loop:
    ...
    if (trynum++ < 20 && n == -1 && (errno == EINTR || errno == EAGAIN)) {
        errorlog("errno %d in recv %d\n", errno, n);
        errno = 0;
        goto loop;
    }

    ("errno %d in recv out %d\n", errno, n);
    // on error disconnect and throw SocketException
    Disconnect();
    throw ServerSocket::Exception("recv error");
    */
    ...
    )
{
    ...
    for ( n = 0; n < m_rgServerHash.size(); ++n) {
    ...
    )
{
    ...
    for (int  n = 0; n < a_nCount; ++n) {
        ...
        catch (const ServerSocket::Exception &e) {
